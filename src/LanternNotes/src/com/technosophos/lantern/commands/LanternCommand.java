...
import static com.technosophos.lantern.servlet.ServletConstants.REQ_PARAM_SESSION;
...
import com.technosophos.lantern.commands.auth.AuthenticationException;
import com.technosophos.lantern.servlet.LanternSession;
import com.technosophos.lantern.types.admin.RepoDescriptionEnum;
 ...
 *
 */
public abstract class LanternCommand extends AbstractCommand {
	
	protected LanternSession ses = null;
	 ...
	 * <ul>
	 * <li>It retrieves the session and does any necessary setup. See {@link LanternSession}</li>
	...
	public void doCommand(Map<String, Object> params, List<CommandResult> results) throws ReRouteRequest {
		...
		this.ses = (LanternSession)params.get(REQ_PARAM_SESSION);
		
		if( !this.checkAuth() ) {
			String friendlyErrMsg = "You must login first.";
			String errMsg = "LanternCommand: Unauthenticated user access.";
	...
	protected boolean userCanWriteRepo(DocumentRepository r) {
		...
		try {
			...
			System.err.println("LanternCommand: Can't find repo UUID.");
	 ...
	 * <p>LanternCommand automatically checks to see if a user is authenticated (unless the 
	 * {@link DIR_NO_AUTH} directive is set in the commnad configuration). This method performs the 
	 * check. By default, it simply calls {@link LanternSession.userLoggedIn()} method.</p>
	 ...
	 */
	protected LanternSession getSession() {
		return this.ses;
	}

	/**
	 * Take a string containing comma-separated tags, and convert it to canonical tags.
	 * This normalizes tags, too.
	 * @param tags String of comma separated tags
	 * @return List of normalized tags.
	 * 
	 */
	public List<String> prepareTags(String tags) {
		String[] t = tags.split(",",512);
		//System.out.format("@@@@@ I just split tags into %d", t.length );
		return this.normalizeTags(Arrays.asList(t));
	}
	
	/**
	 * Normalize a list of tags.
	 * Normalizing involves (at least) scrubbing, lowercasing, and trimming each tag.
	 * @param tags List of tags
	 * @return List of normalized tags
	 * @see Scrubby.cleanText(String)
	 */
	public List<String> normalizeTags(List<String> tags) {
		for(int i =0; i < tags.size(); ++i)
			tags.set(i, Scrubby.cleanText(tags.get(i).trim().toLowerCase()));
		return tags;
	}
	
	/**
	 * Get the value of "doc" from the params.
	 * @return The document ID or an empty string.
	 */
	public String getDocIDFromParams() {
		return this.getFirstParam("doc","").toString();
	}
}
